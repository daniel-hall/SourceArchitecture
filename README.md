...
Lastly, we could have tried the SwiftUI `ObservableObject` protocol approach with the @ObservedObject or @StateObject property wrapper, which sets out to solve the same problems above. However, @ObservedObject doesn't work with UIKit Views or View Controllers, which is a requirement of Source Architecture.
